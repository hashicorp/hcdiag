...
func consulRunners(ctx context.Context, cfg Config, api *client.APIClient, l hclog.Logger) ([]runner.Runner, error) {
	...
	for _, cc := range []runner.CommandConfig{
		{Command: "consul version", Format: "string"},
		{Command: fmt.Sprintf("consul debug -output=%s/ConsulDebug -duration=%s -interval=%s", cfg.TmpDir, cfg.DebugDuration, cfg.DebugInterval), Format: "string"},
		{Command: "consul operator raft list-peers -stale=true", Format: "string"},
	} {
